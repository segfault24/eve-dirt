...
$app->get('/api/lists/', function ($request, $response, $args) {
	...
	$stmt = $db->prepare($sql);
	$stmt->execute(array(':userid'=>$u->getUserId()));
...
$app->post('/api/lists/', function ($request, $response, $args) {
	...
	$stmt = $db->prepare($sql);
	$stmt->execute(array(':listname'=>$listname, ':userid'=>$u->getUserId()));
...
$app->get('/api/lists/{listid}', function ($request, $response, $args) {
	...
	$stmt = $db->prepare($sql);
	$stmt->execute(array(':listid'=>$args['listid']));
...
$app->get('/api/lists/{listid}/types/{typeid}', function ($request, $response, $args) {
	...
	$stmt = $db->prepare($sql);
	$stmt->execute(array(':listid'=>$args['listid'], ':typeid'=>$args['typeid']));
...
$app->put('/api/lists/{listid}/types/{typeid}', function ($request, $response, $args) {
	...
	$stmt = $db->prepare($sql);
	$stmt->execute(array(':typeName'=>$typeName));
	...
	if($quantity == '') {
		...
		$stmt = $db->prepare($sql);
		$stmt->execute(array(':listid'=>$args['listid'], ':typeid'=>$typeid));
		if($stmt->rowCount() == 1) {
			return response;
		}
	...
	$stmt = $db->prepare($sql);
	$stmt->execute(array(':listid'=>$args['listid'], ':typeid'=>$typeid, ':quantity'=>$quantity));
